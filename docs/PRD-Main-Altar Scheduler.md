...
#### 2.5.3 가용성(Availability)설문 링크 보내기
...
#### 2.5.5 자동배정(Auto Assignment) 버튼

- 버튼 활성화 조건 : 선택된 월이 'SURVEY-CONFIRMED'(설문종료) 상태에서만 활성화됨.
- 프로젝트 파일 'PRD 2.5.4 Auto ServerAssignment Logic.md' 파일 내용을 참고함.
...
- 미사(mass_event_id)별 + 일자별 가용성의 2가지 상태관리 : 'AVAILABLE'(가능) / 'UNAVAILABLE'(불가)
- **데이터 저장 구조 (Firestore)**  
