...
이 기능은 Planner가 매달 반복되는 미사 일정 패턴을 미리 설정해두면,
해당 월의 1일부터 말일까지 **요일 반복 방식으로 자동 생성**하여 월간 미사 일정을 초기화한다.
...
| 항목             | 설명                                  |
| -------------- | ----------------------------------- |
| 📅 요일 반복 패턴 적용 | Preset에서 요일별 미사 목록을 읽어 월 전체에 반복 생성  |
| 🔁 전월 의존 제거    | base week 개념 완전 삭제                  |
| 🧹 기존 월 일정 초기화 | 생성 전 기존 mass_events 전체 삭제           |
| 🔒 상태 관리       | 새로 생성되는 미사는 항상 `MASS-NOTCONFIRMED`  |
| 🎛 Planner 통제  | Planner가 Preset 페이지에서 요일별 항목을 직접 편집 |
