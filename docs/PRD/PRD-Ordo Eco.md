...
- **테마 동기화 (Theme Sync)**:
  - 부모 앱(Ordo)은 자식 앱 호출 시 URL Query Parameter로 `?theme=dark|light|system`을 전달합니다.
  - 자식 앱은 **ThemeProvider 마운트 이전**, 즉 `main.tsx` 최상단에서 이 파라미터를 읽어 ThemeProvider의 localStorage 키에 선제 저장해야 합니다.
    이렇게 해야 ThemeProvider가 초기화 시점부터 올바른 테마를 읽어 적용합니다 (useEffect 타이밍 충돌 방지).

  ```typescript
  // main.tsx (ThemeProvider 렌더 직전)
  const _initParams = new URLSearchParams(window.location.search);
  const _initTheme = _initParams.get('theme');
  if (_initTheme === 'dark' || _initTheme === 'light' || _initTheme === 'system') {
    try { localStorage.setItem('altar-ui-theme', _initTheme); } catch {}
  }
  ```

  - ⚠️ **절대 금지**: `App.tsx`의 `useEffect` 내에서 `localStorage.setItem('theme', ...)` 처럼 ThemeProvider와 다른 키로 저장하면 두 시스템이 충돌하여 테마가 무시됩니다.
  - **iframe 모드 동작**: 자식 앱의 설정 UI(`AppSettingsDrawer` 등)에서는 `window.self !== window.top`으로 iframe 여부를 감지하여, iframe 내에서는 화면 설정(테마 변경 버튼)을 숨기고 "Ordo 앱 설정에서 테마를 변경하세요" 안내문을 표시해야 합니다.

- **알림 바로가기 동기화 (Notification Navigation Sync)**:
  - `notifications` 컬렉션 문서에 `click_action` 필드(절대 URL)를 포함하면, Ordo 알림 탭에서 "바로가기 →" 버튼이 자동으로 표시됩니다.
  - `click_action` URL 형식: `https://ordo-altar.web.app/[경로]` (반드시 절대 URL)
  - 바로가기 버튼 동작 흐름: SSO 토큰 생성(`admin_generateCrossAppAuthToken`) → `?authtoken=&theme=` 파라미터 추가 → Ordo AppViewer(`/apps/view?url=...`)로 내부 이동
  - `body` 필드에 URL을 직접 포함시키지 않습니다. URL은 오직 `click_action` 필드로만 전달합니다.
...
- **앱별 데이터 분리 (Data Isolation Strategy)**:
  - 각 자식 앱은 자신의 고유 데이터를 `app_datas/{appId}/` 하위에 격리하여 관리합니다.

  | 앱 | 데이터 루트 경로 |
  | --- | --- |
  | ordo-altar (복사 스케줄러) | `app_datas/ordo-altar/` |
  | ordo-verbum (말씀 앱) | `app_datas/ordo-verbum/` |

- **공유 콘텐츠 (Ordo Contents)**:
  - 생태계 공통 콘텐츠(오늘의 말씀, 대시보드 뉴스 등)는 `ordo_contents/` 루트 컬렉션에서 관리합니다.

  | 경로 | 내용 |
  | --- | --- |
  | `ordo_contents/daily_contents/daily_verses/` | 매일의 복음 말씀 |
  | `ordo_contents/dashboard_news/items/` | 대시보드 공지사항 |

- **생태계 통합 RBAC (`user_app_roles`)**:
  - 사용자의 앱 설치 이력 및 생태계 전역 역할은 `user_app_roles/{uid}` 루트 컬렉션에서 단일 관리합니다.
  - 기존 `users/{uid}/installed_apps/` 서브컬렉션을 대체합니다.
  - 상세 설계: [`PRD-3.4.5-UserAppRoles-RBAC.md`](PRD-3.4.5-UserAppRoles-RBAC.md)

  ```text
  user_app_roles/{uid}
    global_role?: 'superadmin'
    apps: { [appId]: { installed_at, last_accessed? } }
  ```

  > 앱 내부 세부 역할(예: altar의 admin/planner/server)은 각 앱 전용 컬렉션(`memberships` 등)이 담당하며 `user_app_roles`와 혼재하지 않는다.

- **앱 메타데이터 관리**:
  - `sources/master_datas/apps/{appId}` 에 생태계 내 모든 앱의 메타정보(이름, 경로, category 등)를 저장합니다.
  - 관리 UI는 **OrdoAdmin** 앱에서 전담합니다.
