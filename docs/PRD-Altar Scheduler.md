...
- 복사단(server group)이 실제적으로 최상위 운영 담위임(server_groups 컬렉션)
- 경로:
  . `/parish/${parishCode}/server-groups` → 본당 소속 복사단 리스트 페이지
  . 리스트 페이지 내 [복사단 생성] 버튼 클릭 시 `/parish/${parishCode}/server-groups/new` 로 이동
- 동작 흐름:
...
#### 2.6.3 필요 인원(required_servers) 설정
