...
import { Card, Heading } from '@/components/ui'; // Assuming these exist
...
export default function SurveyManagement() {
  ...
  const fetchSurveys = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        const q = query(
          collection(db, COLLECTIONS.SERVER_GROUPS, serverGroupId, 'availability_surveys')
        );
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 dark:bg-transparent flex flex-col">
      <PremiumHeader
        title="설문 관리"
        subtitle="월별 복사 미사참여 설문 현황을 확인할 수 있습니다."
        icon={<ClipboardList size={22} />}
        onBack={() => navigate(-1)}
      />
      <div className="py-4 px-3 sm:px-4 flex-1">
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        {survey.start_date &&
                          dayjs(survey.start_date.toDate()).format('YYYY.MM.DD')}{' '}
                    ...
                    <Badge
                      ...
                      className={
                        survey.status === 'OPEN'
                          ? 'bg-green-600 dark:bg-green-700 dark:text-white'
                          : 'bg-gray-400 dark:bg-gray-600 dark:text-gray-200'
                      }
                    >
                  ...
                  <div className="mt-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          navigate(`/server-groups/${serverGroupId}/surveys/${survey.id}/by-server`)
                        }
                        className="dark:bg-slate-700 dark:border-slate-600 dark:text-gray-200"
                      >
                        복사별 보기
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          navigate(`/server-groups/${serverGroupId}/surveys/${survey.id}/calendar`)
                        }
                        className="dark:bg-slate-700 dark:border-slate-600 dark:text-gray-200"
                      >
                        달력 보기
        ...
        <DrawerContent className="max-h-[85vh] flex flex-col dark:bg-slate-900 dark:border-slate-800">
          <DrawerHeader>
            <DrawerTitle className="dark:text-gray-100">
              {selectedSurvey &&
                `${selectedSurvey.id.slice(0, 4)}년 ${parseInt(selectedSurvey.id.slice(4))}월 설문 현황`}
            </DrawerTitle>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(() => {
                      ...
                      if (validList.length === 0 && bottomList.length === 0) {
                        return (
                          <div className="col-span-2 text-center text-gray-400 py-4 text-xs">
                            표시할 대상이 없습니다.
                          </div>
                        );
                      }
                      ...
                      return (
                        <>
                          {validList.map((m) => (
                            <div
                              key={m.id}
                              className={`p-2 rounded border text-sm flex items-center justify-between ${m.hasRes ? 'bg-blue-50 border-blue-100 dark:bg-blue-900/20 dark:border-blue-800' : 'bg-white border-gray-100 dark:bg-slate-800 dark:border-slate-700'}`}
                            >
                              ...
                              {m.hasRes ? (
                                <Badge
                                  variant="outline"
                                  className="text-[10px] bg-white text-blue-600 border-blue-200 dark:bg-transparent dark:text-blue-300 dark:border-blue-700 px-1 py-0 h-5 shrink-0"
                                >
                                  제출
                                </Badge>
                              ) : (
                          ...
                          {bottomList.length > 0 && (
                            <>
                              ...
                              {bottomList.map((m) => (
                                <div
                                  key={m.id}
                                  className="p-2 rounded border border-gray-200 bg-gray-50 text-sm flex items-center justify-between opacity-60 dark:bg-slate-800/50 dark:border-slate-700"
                                >
                                  <div className="flex items-center gap-1.5 overflow-hidden">
                                  ...
                                  {m.hasRes ? (
                                    <Badge
                                      variant="outline"
                                      className="text-[10px] bg-gray-100 text-gray-500 border-gray-200 dark:bg-transparent dark:text-gray-500 dark:border-gray-700 px-1 py-0 h-5 shrink-0"
                                    >
                                      제출됨
                                    </Badge>
                                  ) : (
          ...
          <DrawerFooter className="dark:bg-slate-900">
            <Button
              variant="outline"
              onClick={closeDrawer}
              className="dark:bg-slate-800 dark:text-gray-200 dark:border-slate-700"
            >
              닫기
            </Button>
          </DrawerFooter>
