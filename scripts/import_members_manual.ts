...
// credential을 찾기 위해 노력합니다.
console.log('Using Project ID: altar-scheduler-dev');
try {
  initializeApp({
  ...
  console.log('Failed to initialize with applicationDefault(), trying without credential option...');
...
async function run() {
  const sgId = 'SG00001';
  const collectionRef = db.collection('server_groups').doc(sgId).collection('members');

  console.log(`Starting upload to ${collectionRef.path}...`);
  // Settings 출력
  const settings = db.settings;
  // console.log('Firestore Settings:', JSON.stringify(settings, null, 2));

  const batch = db.batch();

  MEMBERS_DATA.forEach((m, index) => {
    ...
    // console.log(`Prepared: ${id} ${m.name_kor}`);
  });

  console.log(`Committing batch with ${MEMBERS_DATA.length} documents...`);
  try {
    const result = await batch.commit();
    console.log('✅ Upload result:', result.length, 'writes committed.');
    console.log('!!!!!! SUCCESS !!!!!!');
  } catch (error) {
    console.error('❌ Upload FAILED:', error);
    if ((error as any).code === 7) {
      console.error('Permission Denied error. Please run `gcloud auth application-default login`');
    }
    process.exit(1);
  }
}
...
run().catch((err) => {
  console.error('Global capture:', err);
  process.exit(1);
});
